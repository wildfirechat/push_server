...
### 一，申请厂商推送服务
目前支持小米、华为、荣耀、vivo、OPPO、魅族、苹果 APNs、谷歌 FCM、鸿蒙、个推、UniPush 等推送，需要到各个厂商的开发者后台申请推送相关 key
...
1. 修改数据库和端口配置（如需）
   ...
   编辑 `src/main/resources/application.properties`，根据实际环境修改数据库连接和端口配置。默认使用 MySQL，本地开发可使用 H2（已注释）。
...
3. 运行
    编译成功之后，在 `target` 目录找到 `push-xxxx.jar`，然后执行：
    ```
...
4. 通过后台页面配置推送参数
   ...
   所有推送配置（包括证书、密钥、AppID 等）均通过后台管理页面配置，不再依赖本地配置文件：
   1. 启动服务后，访问 `http://localhost:8086/admin/`
   2. 使用默认账号 `admin` / `admin123` 登录（**首次登录后请立即修改密码**）
   3. 在「配置管理」页面选择对应平台，填写参数并保存
   4. 对于 APNs / FCM 等需要上传证书/凭证的平台，点击字段旁的「上传」按钮上传文件，路径会自动填充
   5. 配置保存后当前节点立即生效，集群其他节点最长约 30 秒后自动同步
