...
推送服务默认监听 **2 个端口**，实现推送接口与管理后台的隔离：

| 端口 | 用途 | 允许访问的路径 |
|------|------|--------------|
| `8085` | 推送服务端口（开放给 IM 服务调用） | `/android/push`、`/ios/push`、`/harmony/push` |
| `8086` | Admin 后台管理端口 | `/api/admin/*`、`/admin/*`（含静态页面） |

**访问方式示例：**
- IM 服务调用推送：`POST http://localhost:8085/ios/push`
- 后台管理页面：`http://localhost:8086/admin/`
- 后台 API：`http://localhost:8086/api/admin/config/apns`

> 如需修改端口，请编辑 `src/main/resources/application.properties` 中的 `server.port` 和 `admin.server.port`。

## 后台管理

所有推送配置（包括证书、密钥、AppID 等）均通过后台页面管理，不再依赖本地配置文件：

1. 启动服务后，访问 `http://localhost:8086/admin/`
2. 使用默认账号 `admin` / `admin123` 登录（**首次登录后请立即修改密码**）
3. 在「配置管理」页面选择对应平台，填写参数并保存
4. 对于 APNs / FCM 等需要上传证书的平台，点击字段旁的「上传」按钮上传文件

**配置验证：** 保存配置或上传证书后，系统会自动验证配置有效性：
- APNs p8 密钥：验证文件格式是否有效
- FCM 凭证：验证 JSON 格式是否有效
- 华为 HMS / 荣耀：向厂商 OAuth 服务器请求 token 验证凭证
- 鸿蒙 HM：验证 RSA 私钥能否正确解析
- 小米 / OPPO / vivo / 个推 / UniPush：验证必填参数是否为空

**集群部署注意：** 配置和证书文件保存在数据库中，各节点通过 30 秒定时刷新自动同步。修改配置后当前节点立即生效，集群其他节点最长约 30 秒后同步。
...
## 接入极光推送
极光推送暂未内置集成，可参考「添加其它推送服务」的说明自行添加，注意同样需要开通厂商推送来实现离线推送。

## 旧版地址
旧版是配置文件版本的，新版本是有管理后台，在管理后台进行配置的。如果需要使用旧版本，请切换到分支```config```。
